...
def translate_model_for_provider(provider: str, model: str) -> str:
    ...
    当前所有 provider 均原样返回。保留此函数作为未来 provider 适配的扩展点。
    """
    return model
...
#     1. 环境变量 AGNES_MAX_TOKENS_CAP / MIMO_MAX_TOKENS_CAP（最高优先级，运维覆盖）
...
def _ssrf_check(url: str) -> None:
    """SSRF 防护：5步法校验 base_url 安全性（best-effort，校验失败仅告警不阻塞）"""
    try:
